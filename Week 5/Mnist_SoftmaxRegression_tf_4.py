...
# Softmax regression classification of the MNIST database in TensorFlow, part 4
...
X = tf.placeholder(tf.float32, shape=(None, ncoeffs), name="X")
Y = tf.placeholder(tf.float32, shape=(None, num_classes), name="Y")

# Initial coefficients & bias

# Start with random values that follow a normal distribution (Xavier's initialization)
W = tf.Variable(tf.truncated_normal([ncoeffs, num_classes], stddev=1/np.sqrt(ncoeffs)))
