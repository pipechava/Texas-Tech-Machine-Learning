...
# Linear regression classification of the MNIST database, part 4
...
rand_scale = 0.01
dim1 = train_data.shape[0]
dim2 = train_data.shape[1]
train_data_w_noise = train_data + np.random.rand(dim1, dim2) * rand_scale

# Plot a few images
"""
for idx in range(5):
  image = train_data_w_noise[idx].reshape(28,28)
  plt.figure()
...
model.fit(train_data_w_noise, train_labels_onehot)
...
for n in range(num_classes):
    ...
    plt.figure()
    plt.imshow(coef_img, vmin=-.05, vmax=.05, cmap="viridis")
